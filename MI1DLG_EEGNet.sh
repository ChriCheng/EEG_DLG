...
# MI1DLG_EEGNet 参数填写区
#
# MI1 单次 DLG 攻击。checkpoint 和 out_dir 留空时会按下面参数自动生成。
# eval_session 留空表示不限制测试 session；indices 留空表示随机选样本。
# iDLG 只支持 batch_size=1；如需验证多张 trial，请改 indices 后分别运行多次。
# ==============================================================================
...
fold_name="seed_${seed}_train_session_${train_session}"
...
echo
echo "================================================================================"
echo "[MI1DLG_EEGNet] dataset=$dataset model=$model checkpoint=$checkpoint"
echo "[MI1DLG_EEGNet] split=$split eval_session=${eval_session:-all} indices=${indices:-random} batch_size=$batch_size"
echo "[MI1DLG_EEGNet] attack_head=$attack_head label_mode=$label_mode iters=$iters lr=$lr optimizer=$optimizer"
echo "[MI1DLG_EEGNet] dummy_init_scale=$dummy_init_scale"
echo "[MI1DLG_EEGNet] epsilon=$epsilon sensitivity=$trial_laplace_sensitivity"
echo "[MI1DLG_EEGNet] plot_channel=$plot_channel waveform_grid=$waveform_grid waveform_font_size=$waveform_font_size"
echo "[MI1DLG_EEGNet] run_name=$run_name out_dir=$out_dir"
echo "================================================================================"
